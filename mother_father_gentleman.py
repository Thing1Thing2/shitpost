# lifted from https://github.com/omackenzie/Terminal-Video-Player
...
while True:
    try:
        height = int(
            input(
                "Enter the height you want the video to be inside the terminal (from 31-256): "
            )
        )
        if height < 31:
...
while True:
    amount_of_frames = input(
        "Enter the amount of frames you want to play (type ALL if you want to play the entire video): "
    )
            ...
            if ret:
                ...
                for y in range(height):
                    for x in range(width):
                        r, g, b = (
                            int(resized[y, x, 2] / 51),
                            int(resized[y, x, 1] / 51),
                            int(resized[y, x, 0] / 51),
                        )
